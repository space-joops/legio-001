...
현재까지 PR #1~#22까지 순차 진행. `main`은 PR #21까지 머지된 상태이고, **PR #22(스플래시 v2 + 기도문 공식 문구 교체)는 아직 리뷰/머지 대기 중**이다.
...
- PR 본문에 "이번 범위에서 제외한 항목"을 항상 명시해왔다. PR #21 기준 남은 후보: localStorage 단일 저장 대비 백업(내보내기) 유도 UX(초기화 전 "먼저 내보내기" 제안 포함), `src/app/secretary/report/page.tsx`(약 800줄) 분리, "제출하기" 용어 전면 재검토, 출석/기도 회차 탭 `role=tablist` 시맨틱, 보고서 내 간부 명단과 전역 명단(3중 데이터) 동기화 문제, `NEXT_PUBLIC_SITE_URL`의 Vercel 설정 여부 확인(사용자 확인 필요).
