...
현재까지 PR #1~#20까지 순차 진행. `main`은 PR #19까지 머지된 상태이고, **PR #20("접힘 섹션 heading 시맨틱 + 배너 터치 타깃·입력 라벨 접근성 보완")은 아직 리뷰/머지 대기 중**이다. 로컬 `claude/korean-legionella-activity-app-78ztzd` 브랜치는 PR #20의 커밋까지 포함하고 원격과 동기화되어 있다.
...
- PR 본문에 "이번 범위에서 제외한 항목"을 항상 명시해왔다. PR #20 기준 남은 후보: localStorage 단일 저장 대비 백업(내보내기) 유도 UX, `src/app/secretary/report/page.tsx`(약 750줄) 분리, `NEXT_PUBLIC_SITE_URL`의 Vercel 설정 여부 확인(사용자 확인 필요).
