...
현재까지 PR #1~#25까지 순차 진행. `main`은 PR #24까지 머지됐고 **릴리즈 `v0.3.0`** 이 생성돼 있다(그 전 태그는 `v0.1.0` 뿐이었다). **PR #25(연간 사업보고서 제7호 + 월례보고서 활동 분류)는 아직 리뷰/머지 대기 중**이다.

### PR #25에서 추가된 것 (버전 0.4.0)
- `src/lib/activityItems.ts` — 사업보고서의 11개 분류·약 50항목. 월례보고서가 `activityTallies`로 매달 기록하고, 공식 월례양식의 "Pr.활동사항" 줄은 0이 아닌 항목만 골라 자동 조립(`formatActivitySummaryLine`).
- `src/lib/annualReportUtils.ts`의 `aggregateYear` — 그 해 월례보고서 12개를 합산. **`refreshAggregate`는 서기가 손으로 넣은 평의회 출석·간부 이동을 보존**해야 한다(앱이 그 데이터를 만들 수 없으므로).
- `/secretary/annual?id=` 라우트 + `AnnualReportPrintView` + `buildAnnualReportRtf`. 진입점은 서기 홈 섹션(BottomNav는 4칸이 차 있어 탭을 늘리지 않았다).
- RTF 헬퍼를 `src/lib/rtf.ts`로 추출해 월례·연간이 공유. **월례 RTF 출력은 바이트 동일해야 한다** — 리팩터 시 회귀 확인 대상.
- **검토용 예시 데이터**: `scripts/generate-sample-data.mjs` → `public/sample-data.json`(선캐시 제외). 미리보기 URL에 **`?demo=1`** 을 붙이면 온보딩 화면과 설정 화면에 [예시 데이터 불러오기]가 뜬다. 2024년 12개월이 실물 33차 수치라 사업보고서를 만들면 원문 값이 그대로 나온다. **스키마를 바꾸면 `npm run sample-data`로 재생성할 것** — 안 하면 예시 파일만 낡는다. E2E 시드를 손으로 짜는 대신 이 파일을 쓰면 빠르다.
- **검산 기준**: 실물 33차 사업보고서(2024년)로 교구 13,846 / 본당 253 / 미사영성체 498 / 묵주기도 8,483 / 52주간 / 간부별 48·52·42·44 / 회계 27,000·913,000·920,000·20,000 / 단원수 7→6 을 전부 재현하는 것을 확인했다. 집계 로직을 고치면 이 값들로 다시 검산할 것.
