...
- **화면 문구**: **한국어 전용이고 다국어 계층은 없다.** 문구는 그 문구가 보이는 파일의 JSX/상수에 직접 한국어로 적는다(`git grep '평일미사참례'` 로 바로 찾을 수 있어야 한다). 여러 화면이 나눠 쓰는 라벨만 `lib/` 에 상수로 둔다 — `PRAYER_ITEMS`(카운터 5종의 `label`/`abbrev`/`unitLabel`), `OFFICER_ROLE_LABEL`(간부 4직책), `WEEKDAY_LABELS`(요일 7개), `SEED_ITEMS`/`SEED_LABELS`(활동·지출 기본 항목). 값 끼워 넣기는 백틱 템플릿(`` `묵주기도 ${n}단` ``).
