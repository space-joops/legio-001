...
import { computeSundayMassBasis } from "../lib/monthlyReportUtils";

describe("computeSundayMassBasis (높은 난이도)", () => {
  it("일반적인 달의 주일 횟수를 올바르게 계산해야 한다 (예: 2026년 6월, 주회 수요일(3))", () => {
    ...
    if (result) {
      expect(result.sundayCount).toBe(4);
      expect(result.total).toBe(20); // 4 * 5 = 20

      const fromStr = result.from.toISOString().split("T")[0];
      const toStr = result.to.toISOString().split("T")[0];
      expect(fromStr).toBe("2026-05-28");
      expect(toStr).toBe("2026-06-24");
    }
  });

  it("연도가 바뀌는 경계(1월)에서도 정상 동작해야 한다 (예: 2024년 1월, 주회 화요일(2))", () => {
    ...
    expect(result).not.toBeNull();
    if (result) {
      expect(result.sundayCount).toBe(5);
      expect(result.total).toBe(50);
    }
  });

  it("윤년의 2월 말일이 포함된 경우도 정상 동작해야 한다 (예: 2024년 3월, 주회 목요일(4))", () => {
    // 2024년 2월은 29일까지 있음.
  ...
  it("유효하지 않은 입력값(형식 오류)일 경우 null을 반환해야 한다", () => {
